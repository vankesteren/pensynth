...
#' @param debug `boolean` whether to print solver info
#'
...
#'
#' @return A list containing `w`, the estimated weights; and
#' `solution`, the result of the optimization; and `qpsolver`. If `osqp`
#' solver has been used, list will also contain `qpsolver`, the `osqp`
#' solver object
#'
...
#'
#' @export
pensynth <- function(X1, X0, v, lambda = 0, opt_pars = NULL, standardize = TRUE, solver = "osqp",
                     debug = FALSE) {
  if (standardize) {
  ...
  if (solver == "osqp") {
    ...
    # Instantiate/run the quadratic program solver
